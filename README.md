...
This project currently uses an environment variable to set the service to use this can be configured by adding a .env file and specifying the REACT_APP_BASE_URL variable. 
At the time of writing the service located at https://api.beta.tab.com.au/v1/tab-info-service does not include the appropriate CORS headers to use the API directly while
developing. This has been specified as part of the package.json and is only used if the current node environment is set to development. For mor information regarding this,
see the section on proxying api requests in development.
...
## Attributions
chariot by Richard Zeid from the Noun Project
Greyhound by Carl Holderness from the Noun Project
equestrian by Aldric Rodríguez from the Noun Project
